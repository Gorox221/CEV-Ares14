...
reagent-effect-condition-guidebook-total-sanity =
    { $max ->
        [2147483648] цель имеет по крайней мере { NATURALFIXED($min, 2) } единиц рассудка
       *[other]
            { $min ->
                [0] цель имеет не более { NATURALFIXED($max, 2) } единиц рассудка
               *[other] цель имеет между { NATURALFIXED($min, 2) } и { NATURALFIXED($max, 2) } единиц рассудка
            }
    }
